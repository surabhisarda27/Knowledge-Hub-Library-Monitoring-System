...
// Serve frontend static files from project root with custom headers
app.use(express.static(path.join(__dirname), {
  setHeaders: (res, filePath) => {
    try {
      // Remove headers that are not recommended if present
      res.removeHeader('X-XSS-Protection');
      res.removeHeader('X-Frame-Options');

      // Cache rules: HTML should not be aggressively cached; static assets can be long-lived
      if (filePath.endsWith('.html')) {
        // Add CSP only for HTML responses (avoid adding CSP to CSS/JS and false positives in scanners)
        res.setHeader('Content-Security-Policy', "default-src 'self' 'unsafe-inline' https:; frame-ancestors 'self';");
        res.setHeader('Cache-Control', 'no-store');
      } else if (filePath.match(/\.(js|css|png|jpg|jpeg|svg|webp|csv)$/)) {
        // long cache for assets; using querystring-based cache-busting in HTML
        res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
      }
    } catch (err) {
      console.warn('setHeaders error', err && err.message);
    }
  }
}));

// Explicitly serve csv_files directory as static to ensure they're accessible
...
const CSV_DIR = __dirname;  // CSV files are at root level, not in a subfolder
